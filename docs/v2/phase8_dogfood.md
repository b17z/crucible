...
| 3 | S2 review | ROUGH: LLM compliance assertions (`spec-missing-auth` etc.) make a live API call and spend tokens on every `crucible review` by default, without `--verify-llm` — the CLI help doesn't flag this default-on behavior as costing tokens the way `--verify-llm` is flagged | fix-this-phase (--llm opt-in landed) | Real UX gap (silent default cost) but not a correctness bug and not trivially rule-decidable as an FP class; needs a documentation/help-text decision (and possibly a default-off discussion) that's bigger than a burn-down-sized fix. |
