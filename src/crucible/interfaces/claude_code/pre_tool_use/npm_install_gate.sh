...
# Verify the runtime can actually parse YAML. We use python3 + PyYAML.
# If either is missing, fail loudly with a clear diagnostic rather than
# silently degrade to "everything's unapproved" behavior — which is
# misleading because the file may have valid approvals the gate
# physically cannot read.
if ! command -v python3 >/dev/null 2>&1; then
    echo "" >&2
    echo "🛑 crucible: python3 not found on PATH" >&2
    echo "" >&2
    echo "The npm_install_gate hook requires python3 to safely parse" >&2
    echo "approved-deps.yaml. Install python3 or remove the hook." >&2
    echo "" >&2
    exit 2
fi
if ! python3 -c "import yaml" 2>/dev/null; then
    echo "" >&2
    echo "🛑 crucible: PyYAML not installed in the python3 environment" >&2
    echo "" >&2
    echo "The npm_install_gate hook requires PyYAML to safely parse" >&2
    echo "approved-deps.yaml. Install it with:" >&2
    echo "  pip install pyyaml" >&2
    echo "" >&2
    echo "Until then, every install is blocked (fail-closed)." >&2
    echo "" >&2
    exit 2
fi
