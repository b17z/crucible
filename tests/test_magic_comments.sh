...
HOOK="$(cd "$(dirname "$0")/.." && pwd)/interfaces/claude_code/user_prompt_submit/magic_comments.sh"
FAILED=0
...
printf '{"user_prompt":"crucible-approve: lodash@4.17.21"}' | bash "$HOOK" >/dev/null 2>&1
...
SCRATCH=$(mktemp -d); cd "$SCRATCH"; mkdir -p .crucible
printf '{"user_prompt":"crucible-approve: @tanstack/react-router@1.0.0"}' | bash "$HOOK" >/dev/null 2>&1
...
SCRATCH=$(mktemp -d); cd "$SCRATCH"; mkdir -p .crucible
printf '{"user_prompt":"crucible-approve: pkg-with-quotes\\"@1.0"}' | bash "$HOOK" >/dev/null 2>&1
...
SCRATCH=$(mktemp -d); cd "$SCRATCH"; mkdir -p .crucible
printf '{"user_prompt":"crucible-approve: foo\\\\bar@1.0"}' | bash "$HOOK" >/dev/null 2>&1
...
SCRATCH=$(mktemp -d); cd "$SCRATCH"; mkdir -p .crucible
printf '{"user_prompt":"crucible-mode: exploration"}' | bash "$HOOK" >/dev/null 2>&1
...
SCRATCH=$(mktemp -d); cd "$SCRATCH"; mkdir -p .crucible
printf '{"user_prompt":"crucible-sign: 1 3"}' | bash "$HOOK" >/dev/null 2>&1
